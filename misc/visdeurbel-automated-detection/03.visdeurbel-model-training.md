...
Training 324 samples for 3 epochs takes roughly **40 minutes** on a consumer GPU with 16 GB VRAM. Note: `train_fast.py` skips validation during training (no `eval_strategy="epoch"`) — run evaluation separately in step 5.
